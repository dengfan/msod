    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        if (splitted[0].equals("!spy")) {
            ...
            for (ChannelServer cservers : csss) {
                Collection<MapleCharacter> cmc = new LinkedHashSet<MapleCharacter>(cservers.getPlayerStorage().getAllCharacters()); // Fix ConcurrentModificationException.
        ...
        //c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getClock(Integer.parseInt(splitted[1])));
            ...
            if (player.getClient().getChannelServer().getMapFactory().destroyMap(mapid)) {
                ...
                Collection<MapleCharacter> mcs = new LinkedHashSet<MapleCharacter>(map.getCharacters()); // do NOT remove, fixing ConcurrentModificationEx.
                outerLoop:
            ...
            mc.dropMessage("All mobs are now " + (false ? "" : "not ") + "Move locked.");
        } else if (splitted[0].equalsIgnoreCase("!split")) {
            Collection<MapleCharacter> lmc = new LinkedHashSet<MapleCharacter>(c.getPlayer().getMap().getCharacters());
