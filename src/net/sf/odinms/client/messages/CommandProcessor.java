        ...
        for (File file : scriptFolder.listFiles()) {
            if (file.isFile() && file.canRead()) {
                ...
                try {
                    ...
                    BufferedReader bf = new BufferedReader(new InputStreamReader(in, EncodingDetect.getJavaEncode(file)));

                    String lines = "load('nashorn:mozilla_compat.js');" + bf.lines().collect(Collectors.joining(System.lineSeparator()));
                    CompiledScript compiled = ((Compilable) command).compile(lines);
                    compiled.eval();
            ...
            if (splitted.length > 0 && splitted[0].length() > 1) {
                ...
                if (definitionCommandPair != null && gmLevel >= definitionCommandPair.getDefinition().getRequiredLevel()) {
                    ...
                    try {
                        ...
                        mc.dropMessage("错误：" + e.getClass().getName() + " " + e.getMessage());
                    ...
                    if (definitionCommandPair == null && c.getPlayer().getGMLevel() > 0) {
                        if (type == 0) {
                            mc.dropMessage("[系统] 命令 " + splitted[0] + " 不存在或你的权限不足");
                        } else {
                            mc.dropMessage("[系统] 命令 " + splitted[0] + " 不存在或你的权限不足");
                        }
                        return true;
