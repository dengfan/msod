...
public class MonsterDropCreator {
    ...
    public static void main(String args[]) throws FileNotFoundException, IOException, NotBoundException, InstanceAlreadyExistsException, MBeanRegistrationException, NotCompliantMBeanException, MalformedObjectNameException {
        MapleData data = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("net.sf.odinms.wzpath", "wz") + "/String.wz")).getData("MonsterBook.img");
        ...
        for (MapleData dataz : data.getChildren()) {
            ...
            if (dataz.getChildByPath("reward").getChildren().size() > 0) { // Fix for Monster without any reward causing SQL error
                sb.append("INSERT INTO monsterdrops (monsterdropid, monsterid, itemid, chance)  VALUES ");
            ...
            if (Pair.getLeft() >= 2380000 && Pair.getLeft() <= 2388054) {
                ...
                for (Pair<Integer, MobInfo> Pair_ : mobCache) {
                    if (Pair_.getRight().getName().equalsIgnoreCase(bookName.toString())) {
                        ...
                        SQL.append("INSERT INTO monsterdrops (monsterdropid, monsterid, itemid, chance) VALUES (DEFAULT, " + Pair_.getLeft() + ", " + Pair.getLeft() + ", " + rate);
                        if (includeQuestDrops) {
                            SQL.append(", DEFAULT);");
                        } else {
                            SQL.append(");");
    ...
    public static void getAllMobs() {
        ...
        MapleDataProvider data = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("net.sf.odinms.wzpath", "wz") + "/String.wz"));
        MapleDataProvider mobData = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("net.sf.odinms.wzpath", "wz") + "/Mob.wz"));
