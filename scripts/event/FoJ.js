...
function announce() {
	...
	em.getChannelServer().broadcastPacket( 
		net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement will begin in " + i + " minutes. Speak to Ria in Ellinia to enter!"));
	i--;
}
...
function mesoDistribution() {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			...
			map.broadcastMessage(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[Event] " + winner.getName() + " wins " + meso + " mesos."));
...
function start() {
	scheduleNew();
	em.getChannelServer().broadcastPacket( 
		net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement has now begun!"));	
	var iter = em.getInstances().iterator();
	while (iter.hasNext()) {
		var eim = iter.next();
		if (eim.getPlayerCount() > 0) {
