...
function playerDisconnected(eim,player) {
	...
	if (player.getName().equals(eim.getProperty("leader"))) {
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6,"此队伍队长已经离开，任务将会被中断。"));
		}
	}
...
function monsterValue(eim,mobId) { // potentially display time of death? does not seem to work
	if (mobId == 8800002) { // 3rd body
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6,"恭喜成功打败大怪物：扎昆"));
		}
	}
	return -1;
}
