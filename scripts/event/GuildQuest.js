...
function begin(eim) {
		...
		var iter = party.iterator();
                while (iter.hasNext()) {
                        iter.next().getClient().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6,"一股神秘的力量使大门打开了。"));
		}
        //}
}
...
function playerDisconnected(eim, player) {
	...
	if (player.getName().equals(eim.getProperty("leader"))) { //check for party leader
		...
		var iter = party.iterator();
                while (iter.hasNext()) {
			var pl = iter.next();
                        pl.getClient().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6,"带领人员离开了，所下的成员不能继续调查古堡。"));
			if (pl.equals(player)) {
				removePlayer(eim, pl);
			}			
			else {
...
function earringcheck(eim, player) {
	var iter = eim.getPlayers().iterator();
		...
		var pl = iter.next();
			...
			pl.addHP(-30000);
			pl.getMap().broadcastMessage(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6,pl.getName() + " 玩家由于没有戴耳环受到邪恶之力的影响死亡。"));
                }
        }
        eim.schedule("earringcheck", 15000);
}
