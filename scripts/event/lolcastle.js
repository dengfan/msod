...
function announce() {
	...
	em.getChannelServer().broadcastPacket( 
		net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[公告事项] 领地的审判副本任务 " + i + " 分钟后开启。"));
	i--;
}
...
function mesoDistribution() {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			...
			map.broadcastMessage(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[公告事项] 恭喜 " + winner.getName() + " 玩家 获得了 " + meso + " 金币"));
...
function start() {
	scheduleNew();
	em.getChannelServer().broadcastPacket( 
		net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[公告事项] 领地的审判副本任务已经开启。"));	
	var iter = em.getInstances().iterator();
	while (iter.hasNext()) {
		var eim = iter.next();
		if (eim.getPlayerCount() > 0) {
...
function playerDead(eim, player) {
	...
	player.getClient().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(1, "你失去了 " + eim.getKillCount(player) + " 点。"));
}
...
function allMonstersDead(eim) {
	...
	while (iter.hasNext()) {
		...
		winner.getClient().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.serverNotice(1, "你获取了 " + eim.getKillCount(winner) + " 点券\r\n你将在2分钟内传送出。"));
