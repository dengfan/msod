	...
	if (selection == 0){
		if (cm.getMeso() < 0) {
 			...
 			cm.sendOk("#v5220007##z5220007#您没有该物品 需要在商城购买后在来吧");
      			cm.dispose();
		} else if (cm.getPlayer().getInventory(Packages.net.sf.odinms.client.MapleInventoryType.getByType(1)).isFull(3)){
...
cm.getC().getSession().write(Packages.net.sf.odinms.tools.MaplePacketCreator.addInventorySlot(type, toDrop)); //刷新背包	
