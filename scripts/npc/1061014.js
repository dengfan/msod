...
function action(mode, type, selection) {   
 ...
 if (pbMap.getCharacters().size() == 0) {
		...
		if (party == null || party.getLeader().getId() != cm.getPlayer().getId()) {
                  ...
                  cm.setBossLog('BF');
                  cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null,net.sf.odinms.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"[Boss公告]" + " : " + "[" + cm.getPlayer().getName() + "]开始挑战巨魔蝙蝠,大家祝福他!",true).getBytes());
                  cm.dispose();
                } else {
