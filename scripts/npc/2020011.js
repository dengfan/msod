    ...
    if (status == 0) {
        ...
        if ((cm.getJob() == 410 || cm.getJob() == 420 || cm.getJob() == 432) && cm.getPlayerStat("LVL") >= 70) {
            if (cm.getJob() != 432 && cm.getPlayerStat("RSP") > (cm.getPlayerStat("LVL") - 70) * 3) {
                ...
                cm.sendNext("来求我？我得卖个关子。");
            }
        } else {
            cm.sendOk("我只能表示爱莫能助了。");
            cm.dispose();
        }
    } else if (status == 1) {
        if (cm.haveItem(4031057, 1)) { // 力气项链
            if (!cm.haveItem(4031058)) { // 智慧项链
                cm.sendOk("不错，你虽然拿到了力气项链，但是现在你还有一个严峻的考验，这次不是武力就能解决的，你还需要有丰富的知识来证明你有第三次转职的能力。\r\n#e前往废矿区的路上，在某面绝壁下面有座黑圣石，找到并通过它的考验并拿到#b智慧项链#k后再回来找我！#n");
                cm.dispose();
            } else {
                ...
                cm.worldMessage(3, "[艾瑞克] : 恭喜" + cm.getPlayer().getName() + "成为一名单身的【" + jobName + "】，单身狗加油吧！");
            ...
            cm.sendOk("你觉得你现在很吊吗？\r\n去找你的二转教官达克鲁并与他单挑，打败他并拿到一张#e#t4031059##n再回来见我，别怕，我为你壮胆。");
            cm.dispose();
        }
    }
}
